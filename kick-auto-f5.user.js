// ==UserScript==
// @name         Kick Auto F5 Pro (Ultimate Aesthetic & DM/Grup Chat Edition v15.2.2 - Whitelist + Timeout Autocomplete + Call Volume Fix)
// @namespace    http://tampermonkey.net/
// @version      15.2.2
// @description  Admin Onay/Red + Beyaz Liste + AI Oto-Onay + Özel Kelime + Timeout (Admin) + Reactions + Last Seen + Profanity Filter + Daily Streak + Read Receipts + Masaüstü Bildirim + Kilit Ekranı + Timeout Autocomplete + Call Volume Ducking
// @author       Sen
...
(function() {
    ...
    let masaustuBildirimAcik = localStorage.getItem('kick_f5_masaustu_bildirim') === 'acik';
    let bildirimIzniVerildi = (typeof Notification !== 'undefined' && Notification.permission === 'granted');
    ...
    function aiAciliDegerlendir(neden, gonderen) {
        ...
        for (const kelime of ozelKelimeler) {
            if (kelime && temiz.includes(kelime)) {
                return {
                    gecerli: true,
                    sebep: `Özel onay kelimesi tespit edildi: "${kelime}"`,
                    ozelKelime: true
                };
            }
        }
        ...
        for (const kelime of AI_GECERSIZ_ANAHTARLAR) {
            if (temiz === kelime || temiz.startsWith(kelime + ' ') || temiz.endsWith(' ' + kelime) || temiz.includes(' ' + kelime + ' ')) {
                return { gecerli: false, sebep: `Şüpheli kelime tespit edildi: "${kelime}"`, ozelKelime: false };
            }
        }
        ...
        if (eslesmeSayisi > 0) {
            return { gecerli: true, sebep: `Acil durum anahtar kelimesi: "${eslesenKelime}" (${eslesmeSayisi} eşleşme)`, ozelKelime: false };
        }
        ...
        if (temiz.length > 15 && kelimeler.length >= 3 && harfSayisi > 10) {
            return { gecerli: false, sebep: 'Acil durum anahtar kelimesi bulunamadı (yetersiz açıklama)', ozelKelime: false };
        }

        return { gecerli: false, sebep: 'Geçersiz veya yetersiz sebep', ozelKelime: false };
    }
    ...
    const CEVIRILER = {
        tr: {
            ...
            confirm_vazgec: 'Vazgeç', confirm_evet: 'Sıfırla',
            ...
            yeni_mesajlar_btn: 'Yeni Mesaj',
            yeni_mesaj_var: 'Yeni mesaj',
            bildirim_ayarlari_baslik: 'Bildirim Ayarları',
            masaustu_bildirim_baslik: 'Masaüstü Bildirimi',
            masaustu_bildirim_aciklama: 'Mesaj ve arama geldiğinde masaüstü bildirimi göster',
            masaustu_bildirim_izin_bekliyor: 'Tarayıcı izni gerekli',
            masaustu_bildirim_izin_ver: 'İzin Ver',
            masaustu_bildirim_izin_verildi: 'İzin verildi',
            masaustu_bildirim_izin_reddedildi: 'İzin reddedildi',
            bildirim_test_btn: 'Test Bildirimi Gönder',
            bildirim_test_baslik: 'Test Bildirimi',
            bildirim_test_metin: 'Masaüstü bildirimleri çalışıyor!',
            ...
            timeout_1saat: '1 Saat',
            timeout_1gun: '1 Gün',
            timeout_1hafta: '1 Hafta',
            timeout_kalici: 'Kalıcı',
            ...
            timeout_locked_by: 'Susturan Yetkili:',
            timeout_locked_remaining: 'Kalan Süre:',
            timeout_locked_reason: 'Sebep:',
            timeout_locked_no_reason: 'Sebep belirtilmedi.',
        ...
        en: {
            ...
            confirm_vazgec: 'Cancel', confirm_evet: 'Reset',
            ...
            yeni_mesajlar_btn: 'New Messages',
            yeni_mesaj_var: 'New message',
            bildirim_ayarlari_baslik: 'Notification Settings',
            masaustu_bildirim_baslik: 'Desktop Notification',
            masaustu_bildirim_aciklama: 'Show desktop notification for messages and calls',
            masaustu_bildirim_izin_bekliyor: 'Browser permission required',
            masaustu_bildirim_izin_ver: 'Allow',
            masaustu_bildirim_izin_verildi: 'Permission granted',
            masaustu_bildirim_izin_reddedildi: 'Permission denied',
            bildirim_test_btn: 'Send Test Notification',
            bildirim_test_baslik: 'Test Notification',
            bildirim_test_metin: 'Desktop notifications are working!',
            ...
            timeout_1saat: '1 Hour',
            timeout_1gun: '1 Day',
            timeout_1hafta: '1 Week',
            timeout_kalici: 'Permanent',
            ...
            timeout_kalan: 'Remaining',
            timeout_dakika: 'min',
            timeout_saat: 'h',
            timeout_gun: 'd',
            timeout_suresiz: 'Permanent',
            ...
            timeout_locked_by: 'Muted By:',
            timeout_locked_remaining: 'Time Remaining:',
            timeout_locked_reason: 'Reason:',
            timeout_locked_no_reason: 'No reason provided.',
    ...
    function masaustuBildirimGoster(baslik, icerik, ikonUrl, zorla = false) {
        if (!masaustuBildirimAcik) return;
        if (typeof Notification === 'undefined') return;
        if (Notification.permission !== 'granted') return;

        if (!zorla && (Date.now() - F5_SAYFA_ACILIS_ZAMANI) < F5_BILDIRIM_SESSIZ_SURESI) return;

        const simdiMs = Date.now();
        const bildirimAnahtari = `${baslik || ''}|${icerik || ''}`;
        const sonGosterim = f5SonBildirimler.get(bildirimAnahtari) || 0;
        if (!zorla && (simdiMs - sonGosterim) < F5_BILDIRIM_TEKRAR_ENGELI) return;
        f5SonBildirimler.set(bildirimAnahtari, simdiMs);
        if (f5SonBildirimler.size > 80) {
            f5SonBildirimler.forEach((v, k) => { if (simdiMs - v > 60000) f5SonBildirimler.delete(k); });
        }

        try {
            const bildirim = new Notification(baslik || 'Kick F5', {
                body: icerik || '',
                icon: ikonUrl || undefined,
                badge: ikonUrl || undefined,
                silent: false,
                tag: 'kick-f5-' + Date.now()
            });
            bildirim.onclick = function() { window.focus(); bildirim.close(); };
            setTimeout(() => { try { bildirim.close(); } catch(e) {} }, 8000);
        } catch(e) {}
    }
    ...
    function zilTonCal(id) {
        try {
            ...
            if (id === 'klasik') {
                notaCal(0, 950, 0.36, 'sine', 0.26);
                notaCal(0.42, 1400, 0.36, 'sine', 0.26);
            } else if (id === 'dijital') {
                [0, 0.15, 0.3].forEach((off, i) => notaCal(off, 600 + i * 220, 0.12, 'square', 0.16));
            } else if (id === 'yumusak') {
                notaCal(0, 784, 1.05, 'sine', 0.22);
                notaCal(0, 1175, 1.05, 'sine', 0.12);
            } else if (id === 'retro') {
                for (let i = 0; i < 4; i++) notaCal(i * 0.1, 520, 0.08, 'sawtooth', 0.12);
            } else if (id === 'marimba') {
                [523.25, 659.25, 783.99, 1046.5].forEach((freq, i) => notaCal(i * 0.13, freq, 0.34, 'triangle', 0.2));
            }
        } catch (e) {}
    }
    ...
    function ozelZilVeriGetir() {
        try {
            const ham = localStorage.getItem('kick_f5_ozel_zil');
            return ham ? JSON.parse(ham) : null;
        } catch (e) { return null; }
    }
    function ozelZilVeriKaydet(ad, dataUrl, sureSn) {
        try { localStorage.setItem('kick_f5_ozel_zil', JSON.stringify({ ad, dataUrl, sure: sureSn })); return true; }
    ...
    function metniSeslendir(metin) {
        ...
        try {
            ...
            soz.onerror = (e) => console.warn('[Kick F5] Seslendirme hatası:', e && e.error);
    ...
    function acilNedeniSesliOku(neden, arayanAd) {
        ...
        if (aktifDil === 'en') {
            cumle = temizNeden
                ? `Emergency call. ${ad ? ad + ' says: ' : ''}${temizNeden}`
                : `Emergency call. ${ad ? ad + ' is calling you.' : 'Someone is calling you.'}`;
        } else {
            cumle = temizNeden
                ? `Acil durum araması. ${ad ? ad + ' diyor ki: ' : ''}${temizNeden}`
                : `Acil durum araması. ${ad ? ad + ' sizi arıyor.' : 'Biri sizi arıyor.'}`;
        }
    ...
    // ==================== BEYAZ LİSTE FONKSİYONLARI (DÜZELTİLDİ) ====================
    ...
    function beyazListedeMi(username) {
        ...
        if (beyazListeCache[guvenliAd] && beyazListeCache[guvenliAd].username) {
            if (beyazListeCache[guvenliAd].username.toLowerCase() === temizAd.toLowerCase()) {
                return true;
            }
        }

        if (beyazListeCache[temizAd] && beyazListeCache[temizAd].username) {
            if (beyazListeCache[temizAd].username.toLowerCase() === temizAd.toLowerCase()) {
                return true;
            }
        }

        const hedefKucuk = temizAd.toLowerCase();
        return Object.values(beyazListeCache || {}).some(v =>
            v && v.username && v.username.toLowerCase() === hedefKucuk
        );
    }
    ...
    function beyazListeyeEkle(username, avatarUrl) {
        ...
        db.ref(`${BEYAZ_LISTE_YOLU}/${guvenliAd}`).set({
            ...
            alert('Beyaz listeye eklenemedi: ' + (err.message || err));
        });
    }
    ...
    function beyazListedenCikar(username) {
        ...
        db.ref(`${BEYAZ_LISTE_YOLU}/${guvenliAd}`).remove().catch(err => {
            console.warn('[Kick F5] Güvenli anahtar ile silme başarısız, alternatif aranıyor:', err);
        });

        Object.entries(beyazListeCache || {}).forEach(([key, val]) => {
            if (val && val.username && val.username.toLowerCase() === temizAd.toLowerCase()) {
                if (key !== guvenliAd) {
                    db.ref(`${BEYAZ_LISTE_YOLU}/${key}`).remove();
                }
            }
        });
    }
    ...
    function beyazListeYukle() {
        ...
        try {
            db.ref(BEYAZ_LISTE_YOLU).on('value', snap => {
                ...
                console.log('[Kick F5] Beyaz liste güncellendi:', Object.keys(beyazListeCache).length, 'kişi');
                const liste = document.getElementById('f5-admin-beyaz-liste');
                if (liste) beyazListeRenderla();
            }, (error) => {
                console.warn('[Kick F5] Beyaz liste okuma hatası (sessiz):', error?.code || error?.message);
            });
        } catch (e) {
    ...
    function globalTimeoutYukle() {
        ...
        try {
            db.ref(GLOBAL_TIMEOUT_YOLU).on('value', snap => {
                ...
                console.warn('[Kick F5] Global timeout okuma hatası (sessiz):', error?.code || error?.message);
            });
        } catch (e) {
    ...
    function globalTimeoutUygula(username, sureMs) {
        if (!db || !username) return;
        const guvenliAd = anahtarGuvenliYap(username);
        if (sureMs === 0) {
    ...
    function timeoutKilitEkraniniGuncelle() {
        ...
        if (benimTimeoutum && tData) {
            ...
            let kalanMetni = '';
            if (tData.bitis === -1) {
                kalanMetni = t('timeout_locked_permanent');
            } else {
            ...
            details.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                    <span style="font-size:12px; color:#888;">${t('timeout_locked_by')}</span>
                    <span style="font-size:13px; font-weight:700; color:#ffb700;">${uygulayan}</span>
                </div>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                    <span style="font-size:12px; color:#888;">${t('timeout_locked_remaining')}</span>
                    <span id="f5-timeout-countdown" style="font-size:20px; font-weight:900; color:#ff4c4c; font-variant-numeric: tabular-nums;">${kalanMetni}</span>
                </div>
                ${tData.sebep ? `
                <div style="margin-top:10px; padding:10px; background:rgba(255,255,255,0.04); border-radius:8px; border:1px solid rgba(255,255,255,0.08);">
                    <div style="font-size:10px; color:#888; margin-bottom:4px;">${t('timeout_locked_reason')}</div>
                    <div style="font-size:12px; color:#ddd; word-break:break-word;">${tData.sebep}</div>
                </div>
                ` : `
                <div style="margin-top:10px; padding:10px; background:rgba(255,255,255,0.04); border-radius:8px; border:1px solid rgba(255,255,255,0.08);">
            ...
            if (timeoutKilitCountdownInterval) {
                clearInterval(timeoutKilitCountdownInterval);
                timeoutKilitCountdownInterval = null;
            }
            if (aktifOdaId) {
    ...
    function acilOnayBekleniyorEkraniGoster(kayitKey, baslik) {
        ...
        document.getElementById('f5-acil-bekleme-iptal').addEventListener('click', () => {
                ...
                if (durum === 'onaylandi') {
                    ...
                    setTimeout(() => {
                        if (f5BekleyenAcilArama) {
                            ...
                            f5BekleyenAcilArama = null;
                            acilDurumModu = true;
                            if (bilgi.tip === 'grup' && bilgi.odaId) {
                                grupAramaBaslat(bilgi.odaId, bilgi.baslik);
                            } else if (bilgi.hedefAd) {
                                dmAramaBaslat(bilgi.hedefAd);
                            }
                        }
                    }, 800);
        ...
        if (db) {
            try {
                db.ref(ACIL_ARAMA_YOLU).push().set({
                    ...
                    odaId: odaId || null,
                    neden: adminMi ? 'Admin - direkt acil arama' : 'Beyaz liste - sebep istenmedi (otomatik onay)',
                    beyazListe: beyazMi,
                    adminAcil: adminMi,
                ...
                console.warn('[Kick F5] Acil kayıt yazılamadı:', e);
            }
        }

        acilDurumModu = true;
        if (grupMu) {
            grupAramaBaslat(odaId, baslik);
        } else if (digerUye) {
            dmAramaBaslat(digerUye);
        }
    }
    ...
    function acilDurumSecimModaliniAc(odaId, baslik, grupMu, digerUye) {
        ...
        document.getElementById('f5-normal-arama-btn').addEventListener('click', () => {
            modalEl.remove();
            if (grupMu) grupAramaBaslat(odaId, baslik);
            else if (digerUye) dmAramaBaslat(digerUye);
        });

        document.getElementById('f5-acil-arama-btn').addEventListener('click', () => {
            modalEl.remove();

            if (mevcutKullaniciAdi === ADMIN_KULLANICI_ADI) {
                acilAramayiDirektBaslat(odaId, baslik, grupMu, digerUye);
                return;
            }

            if (beyazListedeMi(mevcutKullaniciAdi)) {
                acilAramayiDirektBaslat(odaId, baslik, grupMu, digerUye);
                return;
            }
            ...
            document.getElementById('f5-acil-neden-iptal').addEventListener('click', () => nedenModal.remove());
            document.getElementById('f5-acil-neden-gonder').addEventListener('click', () => {
                const neden = nedenInput.value.trim();
                if (neden.length < 3) {
                    nedenHata.textContent = t('acil_neden_hata');
                    return;
                }
                nedenModal.remove();
            ...
            nedenInput.addEventListener('keydown', (e) => {
        ...
        document.getElementById('f5-acil-secim-iptal').addEventListener('click', () => modalEl.remove());
    ...
    stil.innerHTML = `
        ...
        #f5-fab-btn {
            position: fixed; background-color: rgba(20, 21, 24, 0.85); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px);
            border-radius: 50%; display: flex; justify-content: center; align-items: center; cursor: pointer;
            box-shadow: 0 4px 15px rgba(0,0,0,0.6); border: 2px solid rgba(255, 255, 255, 0.05); z-index: 999998;
            user-select: none; transition: transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275), left 0.3s ease-out, top 0.3s ease-out, border-color 0.3s;
        }

        .f5-unread-badge {
            position: absolute; top: -4px; right: -4px; background: #ff4c4c; color: #fff; font-size: 10px; font-weight: bold;
            border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;
            box-shadow: 0 0 8px rgba(255,76,76,0.8); z-index: 10; pointer-events: none; opacity: 0; transform: scale(0);
        ...
        #f5-fab-badge {
            position: absolute; bottom: -24px; left: 50%; transform: translateX(-50%); background: rgba(15, 16, 18, 0.9); backdrop-filter: blur(4px);
            color: var(--f5-main-color); font-size: 11px; font-weight: 700; padding: 3px 8px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1);
            white-space: nowrap; box-shadow: 0 4px 10px rgba(0,0,0,0.6); display: none; pointer-events: none; font-variant-numeric: tabular-nums; transition: color 0.3s;
        }
        ...
        #f5-custom-tooltip {
            position: fixed; background: rgba(15, 16, 18, 0.85); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
            color: #fff; padding: 8px 14px; border-radius: 10px; font-size: 13px; font-weight: 600;
            border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 8px 20px rgba(0,0,0,0.6);
            pointer-events: none; opacity: 0; transition: opacity 0.2s, transform 0.1s; z-index: 9999999; transform: translate(15px, 15px);
        }

        #f5-modal {
            position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 380px;
            background: rgba(20, 20, 20, 0.9); border-radius: 16px; padding: 2px;
            box-shadow: 0 24px 48px rgba(0,0,0,0.8); z-index: 999999; display: none; overflow: hidden;
        ...
        #f5-modal::before {
            ...
            background: conic-gradient(transparent, transparent, transparent, var(--f5-main-color));
            animation: f5-border-spin 3s linear infinite; z-index: 0; opacity: 0.8;
        }
        @keyframes f5-border-spin { 100% { transform: rotate(360deg); } }
        #f5-modal.f5-kenarlik-kapali::before { animation: none; opacity: 0; }

        .f5-modal-content {
            background: var(--f5-panel-bg); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
            border: 1px solid var(--f5-panel-border); border-radius: 14px; padding: 22px; position: relative; z-index: 1; height: 100%; color: #fff; transition: background 0.3s, border-color 0.3s;
        }
        ...
        @keyframes f5-tab-kayma {
            0%, 15% { transform: translateX(0); }
            50%, 62% { transform: translateX(var(--f5-kayma-mesafe, 0px)); }
        ...
        .f5-panel-tema-btn.active .f5-panel-onizleme::after {
            content: '✓'; position: absolute; top: 4px; right: 4px; width: 15px; height: 15px; border-radius: 50%;
            background: #fff; color: #000; font-size: 9px; font-weight: 900; line-height: 1; display: flex; align-items: center; justify-content: center;
        ...
        .f5-aktif-nokta {
            ...
            border-radius: 50%;
            background: var(--f5-main-color);
            border: 2px solid rgba(22,23,27,1);
            box-shadow: 0 0 6px var(--f5-glow-color);
            z-index: 5;
            pointer-events: none;
        ...
        .f5-yeni-mesaj-btn {
            ...
            background: var(--f5-main-color, #53fc18);
            color: #000;
            border: none;
            border-radius: 20px;
            padding: 8px 16px;
            ...
            font-weight: 800;
            cursor: pointer;
            box-shadow: 0 4px 16px rgba(83,252,24,0.6);
            z-index: 100;
            display: flex;
            align-items: center;
            gap: 6px;
        ...
        .f5-yeni-mesaj-btn.show {
            transform: translateX(-50%) scale(1);
            opacity: 1;
            pointer-events: auto;
        }
        .f5-yeni-mesaj-btn:hover {
            filter: brightness(1.1);
            transform: translateX(-50%) scale(1.05);
        }
        .f5-yeni-mesaj-btn .f5-yeni-mesaj-arrow {
            width: 14px; height: 14px;
            display: inline-flex;
            animation: f5-arrow-bounce 1.2s ease-in-out infinite;
        }
        @keyframes f5-arrow-bounce {
        ...
        .f5-beyaz-liste-ekle-btn {
            width: 100%;
            padding: 10px;
            background: linear-gradient(145deg, #53fc18, #3ed10f);
            color: #0a0f07;
            border: none;
            border-radius: 8px;
            font-size: 12px;
        ...
        #f5-timeout-overlay .f5-timeout-lock-icon {
            font-size: 48px; margin-bottom: 16px; display: block;
            animation: f5-lock-pulse 2s ease-in-out infinite;
        }
        @keyframes f5-lock-pulse {
            0%, 100% { transform: scale(1); opacity: 1; }
            50% { transform: scale(1.1); opacity: 0.8; }
        }
        #f5-timeout-overlay .f5-timeout-lock-title {
            font-size: 18px; font-weight: 800; color: #ff4c4c; margin-bottom: 8px;
        }
        #f5-timeout-overlay .f5-timeout-lock-desc {
            font-size: 12px; color: #aaa; margin-bottom: 20px; line-height: 1.5; max-width: 300px;
        }
        #f5-timeout-overlay .f5-timeout-details {
        ...
        .f5-timeout-autocomplete {
            position: absolute; left: 0; right: 0; top: 100%;
            background: var(--f5-panel-bg); border: 1px solid var(--f5-panel-border);
            border-radius: 8px; max-height: 180px; overflow-y: auto;
            z-index: 1000; box-shadow: 0 8px 24px rgba(0,0,0,0.6);
            backdrop-filter: blur(12px);
            margin-top: 4px;
        }
        .f5-timeout-autocomplete-item {
        ...
        .f5-timeout-autocomplete-avatar {
            width: 24px; height: 24px; min-width: 24px;
            border-radius: 50%; background: rgba(255,255,255,0.1);
            overflow: hidden; display: flex; align-items: center; justify-content: center;
            font-size: 10px; font-weight: 700; color: #fff;
        }
        .f5-timeout-autocomplete-avatar img {
            width: 100%; height: 100%; object-fit: cover;
        }
        .f5-timeout-autocomplete-name {
            font-size: 12px; font-weight: 600; color: #fff;
            white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
        }
        .f5-timeout-autocomplete-empty {
            padding: 10px; text-align: center; font-size: 11px; color: #666;
        }
    `;
        ...
        <div id="f5-modal">
            <div class="f5-modal-content">
                ...
                <div id="sekme-ayarlar" class="f5-tab-content">
                    ...
                    <div class="f5-panel-tema-grid">
                        ${panelTemaButonlariHTML}
                    </div>
                    ...
                    <div class="f5-color-picker-row">
                        <input type="color" id="f5-renk-secici" class="f5-color-picker" value="#53fc18">
                    </div>
                    <div class="f5-tema-grid">
                        ${renkTemaButonlariHTML}
                    </div>
                    ...
                    <label class="f5-label"><span>🔔</span> <span data-i18n="bildirim_ayarlari_baslik">${t('bildirim_ayarlari_baslik')}</span></label>

                    <div class="f5-toggle-row">
                        <div class="f5-toggle-metin">
                            <span class="f5-toggle-baslik" data-i18n="masaustu_bildirim_baslik">${t('masaustu_bildirim_baslik')}</span>
                            <span class="f5-toggle-aciklama" data-i18n="masaustu_bildirim_aciklama">${t('masaustu_bildirim_aciklama')}</span>
                            <div id="f5-bildirim-izin-durum" style="margin-top:6px;"></div>
                        </div>
                        <label class="f5-toggle"><input type="checkbox" id="f5-toggle-masaustu-bildirim"><span class="f5-toggle-slider"></span></label>
                    </div>

                    <button id="f5-bildirim-test-btn" class="f5-btn-io f5-ripple-btn" type="button" style="width:100%; margin-bottom:16px;">
                        <span class="f5-btn-io-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" style="width:14px; height:14px;"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg></span>
                        <span data-i18n="bildirim_test_btn">${t('bildirim_test_btn')}</span>
                    </button>
                        ...
                        <div class="f5-chat-main">
                            <div class="f5-chat-main-header">
                                ...
                                <div class="f5-chat-header-actions">
                                    <button id="f5-chat-call-btn" class="f5-chat-call-icon-btn" style="display:none;" title="${t('sesli_arama_btn')}">
                                        ${SVG_TELEFON}
                                    </button>
                            ...
                            <div id="f5-timeout-overlay">
                                <div style="display:flex; flex-direction:column; align-items:center;">
                                    ...
                                    <div class="f5-timeout-details" id="f5-timeout-details">
                ...
                <div id="sekme-admin" class="f5-tab-content">
                    ...
                    <div style="background: rgba(255,76,76,0.08); border: 1px solid rgba(255,76,76,0.3); border-radius: 10px; padding: 12px; margin-bottom: 12px;">
                        ...
                        <div style="display:flex; gap:6px; flex-wrap:wrap; margin-bottom:10px;">
                            <button class="f5-timeout-preset" data-sure="3600000" style="flex:1; min-width:70px; padding:6px 8px; background: rgba(255,183,0,0.15); color:#ffb700; border:1px solid rgba(255,183,0,0.4); border-radius:6px; font-size:10px; font-weight:700; cursor:pointer;" data-i18n="timeout_1saat">${t('timeout_1saat')}</button>
                            <button class="f5-timeout-preset" data-sure="86400000" style="flex:1; min-width:70px; padding:6px 8px; background: rgba(255,183,0,0.15); color:#ffb700; border:1px solid rgba(255,183,0,0.4); border-radius:6px; font-size:10px; font-weight:700; cursor:pointer;" data-i18n="timeout_1gun">${t('timeout_1gun')}</button>
                            <button class="f5-timeout-preset" data-sure="604800000" style="flex:1; min-width:70px; padding:6px 8px; background: rgba(255,183,0,0.15); color:#ffb700; border:1px solid rgba(255,183,0,0.4); border-radius:6px; font-size:10px; font-weight:700; cursor:pointer;" data-i18n="timeout_1hafta">${t('timeout_1hafta')}</button>
                            <button class="f5-timeout-preset" data-sure="-1" style="flex:1; min-width:70px; padding:6px 8px; background: rgba(255,76,76,0.15); color:#ff6b6b; border:1px solid rgba(255,76,76,0.4); border-radius:6px; font-size:10px; font-weight:700; cursor:pointer;" data-i18n="timeout_kalici">${t('timeout_kalici')}</button>
                        </div>

                        <div id="f5-timeout-secili-sure" style="font-size:10px; color:#53fc18; margin-bottom:8px; min-height:14px;"></div>

                        <div style="font-size:10px; font-weight:700; color:#ccc; margin-bottom:6px;" data-i18n="timeout_aktif_liste">${t('timeout_aktif_liste')}</div>
                    ...
                    <div class="f5-toggle-row" style="background: rgba(255,183,0,0.08); border: 1px solid rgba(255,183,0,0.3); border-radius: 10px; padding: 10px; margin-bottom: 12px;">
                        <div class="f5-toggle-metin">
                            ...
                            <span class="f5-toggle-aciklama">Acil arama sebeplerini yapay zeka değerlendirsin (geçerliyse onayla, değilse reddet)</span>
                    ...
                    <div id="f5-ai-kelime-alani" style="display:none; background: rgba(83,252,24,0.06); border: 1px solid rgba(83,252,24,0.25); border-radius: 10px; padding: 10px; margin-bottom: 12px;">
                        ...
                        <div style="font-size:10px; color:#888; margin-bottom:8px;">Bu kelimeler acil arama sebebinde geçerse otomatik onaylanır. Geçmezse AI karar verir.</div>
    ...
    function diliUygula() {
        ...
        if (zamanlayiciKontrol) {
            btnBaslat.textContent = t('btn_durdur');
            durumMetni.textContent = t('durum_aktif');
        } else {
            btnBaslat.textContent = t('btn_baslat');
            durumMetni.textContent = t('durum_kapali');
            if (rozetHerZaman) fabBadge.textContent = t('durum_kapali');
        }
        tooltipText = konumSecici.value === 'custom' ? t('tooltip_surukle') : t('tooltip_varsayilan');
    ...
    const confirmOverlay = document.getElementById('f5-confirm-modal'), confirmMetinEl = document.getElementById('f5-confirm-metin');
    const confirmTamamBtn = document.getElementById('f5-confirm-tamam'), confirmIptalBtn = document.getElementById('f5-confirm-iptal');
    ...
    const fabBtn = document.getElementById('f5-fab-btn'), fabBadge = document.getElementById('f5-fab-badge'), fabIcon = fabBtn.querySelector('.f5-icon'), progressRing = document.getElementById('f5-ring');
    const unreadBadge = document.getElementById('f5-unread-badge');
    const modal = document.getElementById('f5-modal'), btnBaslat = document.getElementById('f5-baslat'), btnSifirla = document.getElementById('f5-sifirla');
    const durumMetni = document.getElementById('f5-durum'), hedefZamanMetni = document.getElementById('f5-hedef-zaman');
    const boyutSlider = document.getElementById('f5-boyut-slider'), boyutDeger = document.getElementById('f5-boyut-deger'), renkSecici = document.getElementById('f5-renk-secici'), konumSecici = document.getElementById('f5-konum-secici');
    const geriSayimKutusu = document.getElementById('f5-countdown-container'), geriSayimMetni = document.getElementById('f5-countdown-timer');
    ...
    const tabButtons = document.querySelectorAll('.f5-tab-btn'), tabContents = document.querySelectorAll('.f5-tab-content');
    ...
    tabButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.getElementById(hedefSekme).classList.add('active');
            requestAnimationFrame(kayanSekmeMesafesiniHesapla);
            ...
            if (hedefSekme === 'sekme-sohbet') {
                ...
                if(aktifOdaId && okunmayanOdalar[aktifOdaId]) {
                    ...
                    if(oItem) {
                        oItem.querySelector('.f5-chat-room-unread').style.display = 'none';
    ...
    if (aiToggleEl) {
        ...
        aiToggleEl.addEventListener('change', (e) => {
            ...
            aiModDurumunuGuncelle();
            aiKelimeAlaniniGuncelle();
            console.log('[Kick F5] AI Modu:', aiModAktif ? 'AÇIK' : 'KAPALI');
        });
    }
    ...
    // ==================== ADMIN TIMEOUT ARAYÜZÜ (AUTOCOMPLETE EKLENDİ) ====================
    ...
    function timeoutAutocompleteGoster(aramaMetni) {
        ...
        filtreli.forEach(k => {
            ...
            item.innerHTML = `
                <div class="f5-timeout-autocomplete-avatar">${avatarIc}</div>
                <div class="f5-timeout-autocomplete-name">${k.username}</div>
            `;
    ...
    if (timeoutUserInput) {
        timeoutUserInput.addEventListener('focus', () => {
            timeoutAutocompleteGoster(timeoutUserInput.value);
        });
        timeoutUserInput.addEventListener('input', () => {
            timeoutAutocompleteGoster(timeoutUserInput.value);
        });
    ...
    document.querySelectorAll('.f5-timeout-preset').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            let sureMetin = '';
            if (seciliTimeoutSure === -1) sureMetin = t('timeout_kalici');
            else if (seciliTimeoutSure === 3600000) sureMetin = t('timeout_1saat');
            else if (seciliTimeoutSure === 86400000) sureMetin = t('timeout_1gun');
            else if (seciliTimeoutSure === 604800000) sureMetin = t('timeout_1hafta');
            if (timeoutSeciliSureEl) timeoutSeciliSureEl.textContent = '✓ ' + sureMetin;
        });
    });

    if (timeoutUygulaBtn) {
        timeoutUygulaBtn.addEventListener('click', () => {
            const username = (timeoutUserInput && timeoutUserInput.value.trim()) || '';
            if (!username) {
                alert('Lütfen bir kullanıcı adı girin!');
                return;
            }
            globalTimeoutUygula(username, seciliTimeoutSure);
    ...
    function profanityListesiRenderla() {
        ...
        liste.forEach(kelime => {
            ...
            item.querySelector('.f5-prof-del').addEventListener('click', () => {
    ...
    if (profEkleBtn && profInput) {
        profEkleBtn.addEventListener('click', () => {
    ...
    const btnModSaat = document.getElementById('btn-mod-saat'), btnModAralik = document.getElementById('btn-mod-aralik'), btnModSeri = document.getElementById('btn-mod-seri');
    function moduDegistir(mod) {
        aktifMod = mod;
        ['saat', 'aralik', 'seri'].forEach(m => {
            document.getElementById(`btn-mod-${m}`).classList.remove('active');
            document.getElementById(`kutu-mod-${m}`).style.display = 'none';
        });
        document.getElementById(`btn-mod-${mod}`).classList.add('active');
        document.getElementById(`kutu-mod-${mod}`).style.display = 'block';
    }
    btnModSaat.addEventListener('click', () => moduDegistir('saat'));
    btnModAralik.addEventListener('click', () => moduDegistir('aralik'));
    btnModSeri.addEventListener('click', () => moduDegistir('seri'));
    ...
    const kayitliBoyut = localStorage.getItem('kick_f5_btn_boyut') || '52', kayitliRenk = localStorage.getItem('kick_f5_btn_renk') || '#53fc18';
    ...
    const panelOpaklikSlider = document.getElementById('f5-panel-opaklik-slider'), panelOpaklikDeger = document.getElementById('f5-panel-opaklik-deger');
    ...
    const renk1Input = document.getElementById('f5-ozel-renk-1'), renk2Input = document.getElementById('f5-ozel-renk-2'), renk3Input = document.getElementById('f5-ozel-renk-3');
    const renk3Temizle = document.getElementById('f5-ozel-renk-3-temizle'), ozelOnizleme = document.getElementById('f5-ozel-tema-onizleme');
    const ozelIsimInput = document.getElementById('f5-ozel-tema-isim'), ozelKaydetBtn = document.getElementById('f5-ozel-tema-kaydet'), ozelHataEl = document.getElementById('f5-ozel-tema-hata');
    let renk3Aktif = true;
    ...
    const kayitliPanelBg = localStorage.getItem('kick_f5_panel_bg'), kayitliPanelBorder = localStorage.getItem('kick_f5_panel_border');
    ...
    const glowSlider = document.getElementById('f5-glow-slider'), glowDeger = document.getElementById('f5-glow-deger');
    ...
    const toggleAnim = document.getElementById('f5-toggle-anim'), toggleBadge = document.getElementById('f5-toggle-badge'), toggleTension = document.getElementById('f5-toggle-tension'), toggleKenarlik = document.getElementById('f5-toggle-kenarlik'), toggleGurultu = document.getElementById('f5-toggle-gurultu');
    ...
    const toggleMasaustuBildirim = document.getElementById('f5-toggle-masaustu-bildirim');
    const bildirimIzınDurum = document.getElementById('f5-bildirim-izin-durum');
    const bildirimTestBtn = document.getElementById('f5-bildirim-test-btn');

    function bildirimIzınDurumGuncelle() {
        if (!bildirimIzınDurum) return;
        if (typeof Notification === 'undefined') {
            bildirimIzınDurum.innerHTML = `<span class="f5-bildirim-durum kapali">Tarayıcı desteklemiyor</span>`;
            return;
        }
        if (Notification.permission === 'granted') {
            bildirimIzınDurum.innerHTML = `<span class="f5-bildirim-durum acik">${t('masaustu_bildirim_izin_verildi')}</span>`;
            if (!masaustuBildirimAcik) {
                masaustuBildirimAcik = true;
                localStorage.setItem('kick_f5_masaustu_bildirim', 'acik');
                if (toggleMasaustuBildirim) toggleMasaustuBildirim.checked = true;
            }
        } else if (Notification.permission === 'denied') {
            bildirimIzınDurum.innerHTML = `<span class="f5-bildirim-durum kapali">${t('masaustu_bildirim_izin_reddedildi')}</span>`;
            masaustuBildirimAcik = false;
            localStorage.setItem('kick_f5_masaustu_bildirim', 'kapali');
            if (toggleMasaustuBildirim) toggleMasaustuBildirim.checked = false;
        } else {
            bildirimIzınDurum.innerHTML = `<span class="f5-bildirim-durum bekliyor">${t('masaustu_bildirim_izin_bekliyor')}</span>`;
        }
    }

    if (toggleMasaustuBildirim) {
        toggleMasaustuBildirim.checked = masaustuBildirimAcik;
        bildirimIzınDurumGuncelle();

        toggleMasaustuBildirim.addEventListener('change', async (e) => {
            masaustuBildirimAcik = e.target.checked;
            localStorage.setItem('kick_f5_masaustu_bildirim', masaustuBildirimAcik ? 'acik' : 'kapali');

            if (masaustuBildirimAcik) {
                if (typeof Notification === 'undefined') {
                    masaustuBildirimAcik = false;
                    toggleMasaustuBildirim.checked = false;
                    localStorage.setItem('kick_f5_masaustu_bildirim', 'kapali');
                    alert('Bu tarayıcı masaüstü bildirimini desteklemiyor.');
                    return;
                }
                if (Notification.permission !== 'granted') {
                    try {
    ...
    if (bildirimTestBtn) {
        bildirimTestBtn.addEventListener('click', async () => {
            if (typeof Notification === 'undefined') {
                alert('Bu tarayıcı masaüstü bildirimini desteklemiyor.');
                return;
            }
            ...
            try {
                f5SonBildirimler.clear();
                const n = new Notification(t('bildirim_test_baslik'), {
                    body: t('bildirim_test_metin'),
                    silent: false
                });
                setTimeout(() => { try { n.close(); } catch(e) {} }, 5000);
            } catch(e) {
                console.warn('[Kick F5] Test bildirim hatası:', e);
            }
        });
    }
    ...
    let kayitliX = localStorage.getItem('kick_f5_btn_x'), kayitliY = localStorage.getItem('kick_f5_btn_y'), kayitliKonum = localStorage.getItem('kick_f5_btn_konum');
    ...
    let surukleniyor = false, yerDegistirdi = false, baslangicX, baslangicY, btnBaslangicX, btnBaslangicY;
    ...
    document.getElementById('f5-kapat').addEventListener('click', () => modal.style.display = 'none');
    document.addEventListener('click', (e) => {
        const imageModalEl = document.getElementById('f5-image-modal');
        if (imageModalEl && imageModalEl.contains(e.target)) return;
        // Acil arama modalları açıkken paneli kapatma
        const acilSecim = document.getElementById('f5-acil-secim-modal');
        const acilNeden = document.getElementById('f5-acil-neden-modal');
        const acilBekleme = document.getElementById('f5-acil-bekleme-modal');
        const acilSonuc = document.getElementById('f5-acil-sonuc-modal');
        if (acilSecim && acilSecim.contains(e.target)) return;
        if (acilNeden && acilNeden.contains(e.target)) return;
        if (acilBekleme && acilBekleme.contains(e.target)) return;
        if (acilSonuc && acilSonuc.contains(e.target)) return;

        if (modal.style.display === 'block' && !modal.contains(e.target) && !fabBtn.contains(e.target) && !confirmOverlay.contains(e.target)) {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.altKey && e.key.toLowerCase() === durdurmaTusu) {
            e.preventDefault();
            if (zamanlayiciKontrol) { sayaciKapat(); alert(t('acil_fren_mesaj')); }
        }
    });
    ...
    function sayaciBaslat(hedefEpoch, metinGosterim, baslangicEpoch, hizliMod = false) {
        ...
        localStorage.setItem('kick_f5_aktif_hedef', hedefEpoch); localStorage.setItem('kick_f5_hedef_metin', metinGosterim); localStorage.setItem('kick_f5_baslangic_zaman', baslangicEpoch);
    ...
    const kHedef = localStorage.getItem('kick_f5_aktif_hedef'), kMetin = localStorage.getItem('kick_f5_hedef_metin'), kBaslangic = localStorage.getItem('kick_f5_baslangic_zaman'), kSeri = localStorage.getItem('kick_f5_seri_sure');
    ...
    // ==================== ADMIN TIMEOUT KONTROL (MESAJ GÖNDERİMİNDE) ====================
    function kullaniciAdminTarafindanSusturulmusMu(username) {
        return kullaniciGlobalSusturulmusMu(username);
    }
    ...
    document.addEventListener('visibilitychange', () => {
        if (sekmeGorunurMu()) bekleyenOkunduMesajlariniIsle();
    });
    ...
    function okunmayanBildirimGuncelle() {
        ...
        Object.keys(okunmayanOdalar).forEach(id => {
            if (!isMuted(id)) {
                toplam += okunmayanOdalar[id];
            } else {
                okunmayanOdalar[id] = 0;
            }
        });
        if(toplam > 0) {
    ...
    async function aktifKullaniciAdiniAl() {
        ...
        if (kayitliNick && kayitliNick.trim()) {
            return { ad: kayitliNick.trim(), avatar };
        }
    ...
    if (yeniMesajBtn) {
        yeniMesajBtn.addEventListener('click', asagiKaydir);
    }
    ...
    function dmOnlineDurumuGuncelle() {
        ...
        if (cevrimIci) {
            ...
            if (kullaniciObj && kullaniciObj.sonGorulme) {
                sonGorulmeMs = kullaniciObj.sonGorulme;
            }
            ...
            if (sonGorulmeMs) {
                ...
                if (dk < 1) {
                    metin = aktifDil === 'tr' ? 'Az önce çevrimiçiydi' : 'Last seen just now';
                } else if (dk < 60) {
                    metin = aktifDil === 'tr' ? `Son görülme: ${dk} dakika önce` : `Last seen: ${dk} min ago`;
                } else if (saat < 24) {
                    metin = aktifDil === 'tr' ? `Son görülme: ${saat} saat önce` : `Last seen: ${saat} h ago`;
                } else if (gun < 30) {
                    metin = aktifDil === 'tr' ? `Son görülme: ${gun} gün önce` : `Last seen: ${gun} days ago`;
                } else {
                    metin = aktifDil === 'tr' ? 'Uzun süre önce' : 'Long time ago';
                }
            } else {
    ...
    document.querySelectorAll('.f5-mute-opt').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (hours === -1) {
                sessizeAlinanlar[aktifOdaId] = -1;
            } else {
                sessizeAlinanlar[aktifOdaId] = Date.now() + (hours * 3600000);
            }
            sessizKaydet();
    ...
    optDelete.addEventListener('click', async () => {
        const onay = confirm(t('sohbet_sil_onay'));
        if(!onay) return;
        optMenu.classList.remove('show');
    ...
    optLeave.addEventListener('click', async () => {
        ...
        db.ref(`sohbetOdasi/${oankiOda}`).once('value', snap => {
            ...
            const güncellemeler = { uyeler: yeniUyeler };
            ...
            if (oda.adminler && oda.adminler[guvenliAd]) {
                ...
                if (yeniUyeler.length > 0) {
                    if (oda.kurucu === mevcutKullaniciAdi) {
                        ...
                        güncellemeler.kurucu = yeniKurucu;
                ...
                güncellemeler.adminler = yeniAdminler;
            }

            db.ref(`sohbetOdasi/${oankiOda}`).update(güncellemeler);
    ...
    // ==================== DÜZELTİLMİŞ MESAJ İÇERİK OLUŞTURMA (Reactions Fix) ====================
    ...
    function typingGosterGuncelle(kullaniciAdi) {
        ...
        row.innerHTML = `
            <div class="f5-chat-msg-avatar">${avatarIcerik}</div>
            <div class="f5-chat-typing-bubble">
                <span class="f5-chat-typing-dot"></span><span class="f5-chat-typing-dot"></span><span class="f5-chat-typing-dot"></span>
            </div>
        `;
    ...
    function odayaGec(odaId, baslik, grupMu, digerUyeAd = null, avatarUrl = null) {
        ...
        if (!grupMu && digerUyeAd) {
            ...
            if (halaAktif) {
                localStorage.removeItem('kick_f5_last_seen_' + anahtarGuvenliYap(digerUyeAd));
            } else {
                const mevcutKayit = localStorage.getItem('kick_f5_last_seen_' + anahtarGuvenliYap(digerUyeAd));
                if (!mevcutKayit) {
                    localStorage.setItem('kick_f5_last_seen_' + anahtarGuvenliYap(digerUyeAd), Date.now().toString());
                }
            }
        }
        ...
        if (grupMu) {
            chatActiveDesc.textContent = 'Grup Sohbeti';
        } else {
            dmOnlineDurumuGuncelle();
        }
        ...
        if (chatCallBtn) {
            ...
            chatCallBtn.onclick = () => {
                if (aktifCagriTuru) { alert(t('cagri_zaten_aramada')); return; }
        ...
        loadMoreBtn.addEventListener('click', () => {
            ...
            db.ref(`mesajlar/${aktifOdaId}`).orderByKey().endAt(enEskiMesajKey).limitToLast(SAYFA_BASI_MESAJ + 1).once('value', snap => {
                ...
                if(msgs.length > 0 && msgs[msgs.length - 1].key === enEskiMesajKey) {
                    msgs.pop();
                }
        ...
        aktifOdaListener.on('child_added', snapshot => {
            ...
            if (!enEskiMesajKey || msgKey < enEskiMesajKey) {
                enEskiMesajKey = msgKey;
            }
        ...
        db.ref(`mesajlar/${odaId}`).limitToLast(SAYFA_BASI_MESAJ).once('value', snap => {
            if (snap.numChildren() >= SAYFA_BASI_MESAJ) {
    ...
    if (messagesEl) {
        messagesEl.addEventListener('scroll', () => {
            ...
            if (isAtBottom && yeniMesajSayaci > 0) {
                yeniMesajSayaciniSifirla();
            }
        });
    }
    ...
    function gifDurumGoster(metin) {
        gifDurumEl.textContent = metin || '';
    }

    async function gifGetir(sorgu) {
        if (!GIPHY_API_KEY || GIPHY_API_KEY === 'BURAYA_GIPHY_API_ANAHTARINIZI_YAZIN') {
            gifGridEl.innerHTML = '';
            gifDurumGoster('⚠️ Önce script içine bir GIPHY API anahtarı eklemelisin (developers.giphy.com).');
            return;
        }
        ...
        try {
            ...
            console.error('[Kick F5] GIPHY API hatası:', hata);
        }
    }
    ...
    gifKapatBtn.addEventListener('click', () => { gifModal.style.display = 'none'; });
    ...
    async function mesajSil(msgKey) {
        ...
        db.ref(`mesajlar/${aktifOdaId}/${msgKey}`).update({ silindi: true, metin: '', tip: 'metin' })
            .catch(err => alert("Mesaj silinemedi! Hata: " + err.message));
        if (duzenlenenMesajKey === msgKey) { chatInput.value = ''; replyEditPreviewGizle(); }
    }

    function tepkiVer(msgKey, emoji) {
        if (!db || !aktifOdaId || !mevcutKullaniciAdi) {
            console.warn('[Kick F5] Tepki verilemedi: db/oda/kullanıcı eksik');
            return;
        }
        if (!msgKey || !emoji) {
            console.warn('[Kick F5] Tepki verilemedi: msgKey veya emoji eksik');
            return;
        }
        const m = mesajCache[msgKey];
        if (!m) {
            console.warn('[Kick F5] Tepki verilemedi: mesaj cache\'de yok', msgKey);
            return;
        }
        if (m.silindi) return;
    ...
    function mesajGonder(icerik = null, tip = 'metin') {
        ...
        if (guncelOdaTuru === 'dm' && guncelDigerUye && engellenenKisiler.includes(guncelDigerUye)) {
            return;
        }

        if (guncelOdaTuru === 'dm' && digerTarafBeniEngelledi) {
            alert(t('engellendi_gonderim_hata'));
            engelDurumunuUygula();
            return;
        }
        ...
        if (kullaniciGlobalSusturulmusMu(mevcutKullaniciAdi)) {
            ...
            alert(`Susturuldunuz! Mesaj gönderemezsiniz. (${sureMetin})`);
            return;
        }
        ...
        if (duzenlenenMesajKey && tip === 'metin') {
            ...
            db.ref(`mesajlar/${aktifOdaId}/${key}`).update({ metin: metin, duzenlendi: true })
                .catch(err => alert("Mesaj düzenlenemedi! Hata: " + err.message));
            return;
        }
        ...
        db.ref(`mesajlar/${aktifOdaId}`).push(yeniMesaj)
            ...
            .catch(err => alert("Mesaj iletilemedi! Hata: " + err.message));
    }
                        ...
                        db.ref(`mesajlar/${odaId}`).orderByChild('timestamp').limitToLast(1).on('child_added', (mSnap) => {
                            ...
                            if (m && m.gonderen !== mevcutKullaniciAdi) {
                                ...
                                if(onSaniyeIcinde && !sohbetSessizMi && (odaId !== aktifOdaId || sekmeArkaPlanda)) {
                                    bildirimSesiCal();
                                    if (masaustuBildirimAcik && Notification.permission === 'granted' && odaId !== aktifOdaId) {
                                        const gonderenObj = tumAktifKullanicilar.find(u => u.username === m.gonderen);
                                        const ikonUrl = (gonderenObj && gonderenObj.avatar) ? gonderenObj.avatar : null;
                                        let icerik = '';
                                        if (m.tip === 'metin') icerik = kufurFiltrele(m.metin);
                                        ...
                                        masaustuBildirimGoster(m.gonderen, icerik, ikonUrl);
                                    }
                                }
                            }
                        });
    ...
    optProfile.addEventListener('click', () => {
        if (aktifOdaKurucu !== mevcutKullaniciAdi && !benAdminMiyim()) { alert(t('admin_gerekli_uyari')); return; }
        const link = prompt("Grubun yeni profil fotoğrafı için resim linki girin (Boş bırakırsanız silinir):");
        if(link !== null) {
    ...
    function grupYonetimPaneliniAc() {
        if (!benAdminMiyim()) { alert(t('admin_gerekli_uyari')); return; }
    ...
    grupDuzenleListe.addEventListener('click', (e) => {
        ...
        if (aktifOdaKurucu !== mevcutKullaniciAdi) {
            alert(aktifDil === 'tr' ? 'Sadece grup kurucusu admin atayabilir!' : 'Only group founder can assign admins!');
            return;
        }
    ...
    document.getElementById('f5-grup-duzenle-onayla-btn').addEventListener('click', () => {
        if (aktifOdaKurucu !== mevcutKullaniciAdi) {
            alert(aktifDil === 'tr' ? 'Sadece grup kurucusu üye/admin yönetebilir!' : 'Only group founder can manage members/admins!');
            return;
        }

        const secilenler = [];
        grupDuzenleListe.querySelectorAll('.f5-uye-checkbox:checked, .f5-yeni-uye-checkbox:checked').forEach(c => secilenler.push(c.value));
        if (secilenler.length === 0) { alert(aktifDil === 'tr' ? 'Grupta en az bir üye kalmalı!' : 'Group must have at least one member!'); return; }

        if (!secilenler.includes(aktifOdaKurucu)) {
            alert(aktifDil === 'tr' ? 'Grup kurucusu gruptan çıkarılamaz!' : 'Group founder cannot be removed!');
            return;
        }
        ...
        alert(aktifDil === 'tr' ? 'Grup üyeleri ve yönetim ayarları güncellendi!' : 'Group members and admin settings updated!');
    });
    ...
    document.getElementById('f5-chat-yeni-grup-btn').addEventListener('click', () => {
    ...
    document.getElementById('f5-grup-kur-kapat').addEventListener('click', () => { grupKurModal.style.display = 'none'; });

    document.getElementById('f5-grup-onayla-btn').addEventListener('click', () => {
        const secilenler = [];
        grupKullaniciSecimi.querySelectorAll('input:checked').forEach(c => secilenler.push(c.value));
        if (secilenler.length < 2) {
            alert('En az 2 kişi seçmelisiniz (Sizinle beraber 3+ kişi)!');
            return;
        }
    ...
    document.getElementById('f5-chat-yeni-dm-btn').addEventListener('click', () => {
    ...
    document.getElementById('f5-dm-sec-kapat').addEventListener('click', () => { dmSecModal.style.display = 'none'; });
        ...
        kayitlar.forEach(k => {
            ...
            satir.innerHTML = `
                ...
                <div style="display:flex; gap:6px;">
                    ${bekliyorMu ? `
                        <button class="f5-admin-onayla-btn" data-oda="${k.odaId || ''}" data-tip="${k.hedefTip}" data-hedef="${k.hedef || ''}" data-hedefad="${k.hedefAd || ''}" data-key="${k.key}" style="flex:1; padding:8px; background: linear-gradient(145deg,#6bff2f,#3ed10f); color:#0a0f07; border:none; border-radius:6px; font-size:11px; font-weight:800; cursor:pointer;">${t('admin_onayla_ara')}</button>
    ...
    if (beyazListeEkleBtn) {
        beyazListeEkleBtn.addEventListener('click', () => {
    ...
    if (beyazListeSecKapat) {
        beyazListeSecKapat.addEventListener('click', () => { beyazListeSecModal.style.display = 'none'; });
    }
    if (beyazListeSecModal) {
        beyazListeSecModal.addEventListener('click', (e) => { if (e.target === beyazListeSecModal) beyazListeSecModal.style.display = 'none'; });
    }
    if (beyazListeArama) {
    ...
    function adminPaneliniAktifEt() {
        ...
        db.ref(ACIL_ARAMA_YOLU).orderByChild('timestamp').limitToLast(1).once('value', ilkAcilSnap => {
            ...
            db.ref(ACIL_ARAMA_YOLU).orderByChild('timestamp').limitToLast(1).on('child_added', (snap) => {
                ...
                if (kayitZamani && kayitZamani < (F5_SAYFA_ACILIS_ZAMANI - 2000)) {
                    return;
                }
                ...
                if (aiModAktif && veri.durum === 'bekliyor') {
                    ...
                    const etiket = degerlendirme.ozelKelime ? 'ÖZEL KELİME' : 'AI';
                    console.log(`[${etiket}] Değerlendirme: "${veri.neden}" → ${degerlendirme.gecerli ? 'GEÇERLİ' : 'GEÇERSİZ'} (${degerlendirme.sebep})`);
                ...
                // Admin panelinde TTS okuma kaldırıldı (isteğe göre)
                ...
                if (veri.durum === 'bekliyor' && !okundular[key]) {
                    ...
                    if (masaustuBildirimAcik && Notification.permission === 'granted') {
                        masaustuBildirimGoster(t('admin_yeni_bildirim'), `${veri.gonderen}: ${veri.neden || ''}`, veri.gonderenAvatar || null);
                    }
                }

                console.log(`[ADMIN] ${t('admin_yeni_bildirim')}: ${veri.gonderen} - ${veri.neden}`);
        ...
        console.log('[Kick F5] Admin paneli aktif edildi. AI Modu:', aiModAktif ? 'AÇIK' : 'KAPALI', '| Özel Kelimeler:', aiOzelKelimeleriGetir().length);
    }
    ...
    async function fbBaslat() {
        ...
        try {
            ...
            if (dmVeri) {
                ...
                if (yas > 120000) {
                    await db.ref(`sesliCagri/${guvenliBenTemizle}`).remove();
                }
            }
        } catch(e) {}
        ...
        try {
            ...
            for (const odaId of Object.keys(tumGrupCagrilari)) {
                ...
                if (gcYas > 300000 && !benKatilimci) {
                    await db.ref(`grupCagri/${odaId}`).remove();
                } else if (gcYas > 300000 && benKatilimci) {
                    await db.ref(`grupCagri/${odaId}/katilimcilar/${guvenliBenTemizle}`).remove();
                }
            }
        } catch(e) {}
        ...
        db.ref('.info/connected').on('value', snap => {
            if (snap.val() === true) {
                document.getElementById('f5-fb-durum').textContent = 'Bağlı';
                document.getElementById('f5-fb-durum').style.color = 'var(--f5-main-color)';
                ...
                document.getElementById('f5-fb-durum').textContent = 'Koptu';
                document.getElementById('f5-fb-durum').style.color = '#ff4c4c';
            }
        });
        ...
        db.ref('aktifKullanicilar').on('value', snap => {
            ...
            tumAktifKullanicilar.forEach(k => {
                ...
                if (btn) {
                    btn.addEventListener('click', () => dmSohbetBaslat(kullaniciAdiGoster));
                }
    ...
    // Arama sırasında yayın sesini kısmak için
    ...
    function yayinSesiniKis() {
        ...
        document.querySelectorAll('video, audio').forEach((el, i) => {
            if (el.closest('#f5-modern-container')) return; // kendi panelimizdeki sesleri etkileme
    ...
    function cagriAvatariCiz(url, harfKaynagi) {
        if (url) {
            cagriAvatarWrap.innerHTML = `<img src="${url}" class="f5-call-avatar-img">`;
        } else {
            cagriAvatarWrap.innerHTML = `<span class="f5-call-avatar-harf">${(harfKaynagi || '?').charAt(0).toUpperCase()}</span>`;
        }
    }
    ...
    function gurultuEngelleyiciZinciriKur(stream) {
        try {
            ...
            console.error('[Kick F5] Gürültü engelleyici zinciri kurulamadı, ham ses kullanılacak:', e);
    ...
    async function mikrofonAl() {
        ...
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            const hata = new Error('Bu sayfada mikrofon API\'sine erişim yok.');
            hata.name = 'NotSupportedOrPolicyBlocked';
            throw hata;
        }
        try {
            ...
            console.error('[Kick F5] getUserMedia hatası:', e.name, e.message);
            throw e;
        }
    ...
    function peerOlustur(uzakAnahtar, adayYaziYolu) {
        ...
        pc._adayKuyrugu = [];
        if (gonderilenSesStream) {
            gonderilenSesStream.getTracks().forEach(tr => pc.addTrack(tr, gonderilenSesStream));
        } else if (yerelSesStream) {
            yerelSesStream.getTracks().forEach(tr => pc.addTrack(tr, yerelSesStream));
        }
        pc.onicecandidate = (e) => {
    ...
    function adayDinle(dinleYolu, pc) {
        ...
        db.ref(dinleYolu).on('value', snap => {
            ...
            Object.values(adaylar).forEach(c => {
                if (c) pc.addIceCandidateGuvenli(c);
            });
        });
    }
    ...
    function cagriPanelGoster(durum, isim) {
        ...
        if (acilDurumModu) {
            ...
            if (durum === 'gelen') {
                acilNedeniSesliOku(aktifAcilNeden, isim);
            }

            if (!acilDurumSesCalıyor && !acilDurumSirenTimer) {
                acilDurumSesiniBaslat();
            }
        ...
        if (durum === 'gelen' && !acilDurumModu) { zilCalmayaBasla(); } else { zilCalmayiDurdur(); }

        if (durum === 'giden') {
            cagriDurumEl.textContent = t('cagri_ariyor');
        } else if (durum === 'gelen') {
        ...
        if (aktifCagriTuru === 'grup') {
            grupCagriAvatarDinlemesiBaslat(aktifCagriGrupOdaId, isim);
        } else {
            dmCagriAvatarDinlemesiBaslat(isim);
        }
    }
    ...
    function cagriBaglandi() {
        ...
        if (aktifCagriTuru === 'grup') {
            grupKatilimciSayisiGuncelle(aktifCagriGrupOdaId);
        } else {
    ...
    function cagriBitir() {
        try {
            ...
            if (yerelSesStream) {
                yerelSesStream.getTracks().forEach(tr => tr.stop());
                yerelSesStream = null;
            }
            if (gonderilenSesStream) {
                gonderilenSesStream.getTracks().forEach(tr => tr.stop());
                gonderilenSesStream = null;
            }
            if (gurultuAudioCtx) {
            ...
            if (aktifCagriTuru === 'grup' && aktifCagriGrupOdaId && mevcutKullaniciAdi) {
                try {
                    db.ref(`grupCagri/${aktifCagriGrupOdaId}/katilimcilar/${anahtarGuvenliYap(mevcutKullaniciAdi)}`).remove();
                } catch(e) {}
            }
            ...
            console.error('[Kick F5] cagriBitir hatası:', e);
        }
    }
    ...
    cagriKapatBtn.addEventListener('click', () => {
        try {
            if (aktifCagriTuru === 'dm' && aktifCagriDmKok) db.ref(aktifCagriDmKok).remove();
        } catch(e) { console.warn('[Kick F5] Kapatma hatası:', e); }
        try { cagriBitir(); } catch(e) { console.error('[Kick F5] cagriBitir hatası:', e); }
    });

    cagriIptalBtn.addEventListener('click', () => {
        try {
            if (aktifCagriTuru === 'dm' && aktifCagriDmKok) db.ref(aktifCagriDmKok).remove();
        } catch(e) { console.warn('[Kick F5] İptal hatası:', e); }
        try { cagriBitir(); } catch(e) { console.error('[Kick F5] cagriBitir hatası:', e); }
    });
    ...
    function dmAramaBaslat(hedefKullanici) {
        ...
        mikrofonAl().then(async () => {
            try {
                ...
                console.error('[Kick F5] Arama kurulum hatası:', e);
                alert('Arama başlatılamadı: ' + (e && e.message ? e.message : e));
            ...
            console.error('[Kick F5] Mikrofon erişim hatası:', e && e.name, e && e.message);
            alert(t('cagri_mikrofon_hata') + '\n(' + (e && e.name ? e.name : 'bilinmeyen') + ')');
            aktifCagriTuru = null; aktifCagriDmKok = null; aktifCagriKarsiAd = null;
            cagriOverlay.classList.remove('show');
            cagriAvatarDinlemesiniDurdur();
            yayinSesiniGeriYukle();
    ...
    function dmGelenAramaDinle() {
        ...
        db.ref(`sesliCagri/${guvenliBen}/gelen`).on('value', snap => {
            ...
            if (veri && !aktifCagriTuru) {
                ...
                if (yas > 120000) {
                    db.ref(`sesliCagri/${guvenliBen}`).remove();
                    return;
                }
    ...
    cagriReddetBtn.addEventListener('click', () => {
        try {
            ...
            if (aktifCagriTuru === 'dm' && aktifCagriDmKok) {
                ...
                try {
                    db.ref(`${kapanacakKok}/durum`).set('reddedildi').catch((err) => {
                        console.warn('[Kick F5] Reddetme sırasında hata:', err);
                        try { db.ref(kapanacakKok).remove(); } catch(e) {}
                    });
                } catch(e) {
                    console.warn('[Kick F5] Firebase reddetme hatası:', e);
                }
            }
        } catch(genelHata) {
            console.error('[Kick F5] Arama reddetme hatası:', genelHata);
        }
        try { cagriBitir(); } catch(e) { console.error('[Kick F5] cagriBitir hatası:', e); }
    });
    ...
    cagriKabulBtn.addEventListener('click', async () => {
        ...
        if (aktifCagriTuru === 'grup') {
            try {
                ...
                console.error('[Kick F5] Mikrofon erişim hatası (grup kabul):', e && e.name, e && e.message);
                alert(t('cagri_mikrofon_hata') + '\n(' + (e && e.name ? e.name : 'bilinmeyen') + ')');
                cagriBitir();
                return;
            }
        ...
        try {
            ...
            console.error('[Kick F5] Mikrofon erişim hatası (dm kabul):', e && e.name, e && e.message);
            alert(t('cagri_mikrofon_hata') + '\n(' + (e && e.name ? e.name : 'bilinmeyen') + ')');
            cagriBitir();
            return;
        }
        ...
        try {
            ...
            console.error('[Kick F5] Arama kabul hatası:', e);
            alert('Arama kabul edilemedi: ' + (e && e.message ? e.message : e));
            cagriBitir();
        }
    });
    ...
    async function grupAramaBaslat(odaId, grupAdi) {
        ...
        try {
            ...
            console.error('[Kick F5] Mikrofon erişim hatası (grup başlat):', e && e.name, e && e.message);
            alert(t('cagri_mikrofon_hata') + '\n(' + (e && e.name ? e.name : 'bilinmeyen') + ')');
            return;
        }
        ...
        try {
            ...
            console.error('[Kick F5] Grup arama Firebase hatası:', e);
            alert('Grup araması başlatılamadı: ' + (e && e.message ? e.message : e));
    ...
    function grupCagriGelenDinle(odaId, grupAdi) {
        ...
        db.ref(`grupCagri/${odaId}`).on('value', snap => {
            ...
            if (veri && !benZatenKatilimciMiyim && !aktifCagriTuru) {
                ...
                if (yas > 300000) {
                    db.ref(`grupCagri/${odaId}`).remove().catch(() => {});
                    return;
                }
                ...
                const kokRef = db.ref(`grupCagri/${odaId}`);
                const kokFn = snap2 => {
                    if (snap2.val() === null && aktifCagriTuru === 'grup' && aktifCagriGrupOdaId === odaId && aktifCagriDurumu === 'gelen') {
                        cagriBitir();
                    }
                };
                kokRef.on('value', kokFn);
